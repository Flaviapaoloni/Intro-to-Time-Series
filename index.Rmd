...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{figures/cross.png}
    \caption{Cross section: time variable $\rightarrow$ year 2010, ID variable $\rightarrow$ postcode}
    \label{fig:crpss}
\end{figure}
...
\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{figures/gdp.png}
    \caption{Time series: time variable $\rightarrow$ date, ID variable $\rightarrow$ Netherlands\\ \tiny{Source: \href{https://fred.stlouisfed.org/series/CLVMNACSCAB1GQNL}{\textcolor{blue}{FRED}} Real GDP Netherlands}}
    \label{fig:gdp}
\end{figure}
...
\end{align}
The \textcolor{blue}{current value} is the value of $y$ at time $t$, \textcolor{blue}{$y_t$}.\\\medskip
The \textcolor{blue}{first lag} (or lagged value) is the value of $y_t$ in the previous period, \textcolor{blue}{$y_{t-1}$}.\\\medskip
The \textcolor{blue}{$p$-th lag} of $y$ is its value $p$ periods before $t$, \textcolor{blue}{$y_{t-p}$}. \\\medskip
...
This condition is very helpful for predicting and forecasting the time series, hence it is a relevant condition.\\\medskip
...
\item Expected value: the mean of the series $y_t$ is the long-run average. \medskip
\item Variance: the dispersion or spread of a probability distribution. It is the expected value of the square of the deviation of the variable from its expected value.\medskip
...
```{r, echo=FALSE}
knitr::include_graphics("/Users/flaviapaoloni/Downloads/Intro-to-Time-Series/wn.pdf")
```
...
\end{equation*}
where $\mu$ is an constant, $\sigma^2$ is the variance of $\epsilon_t$.\medskip
...
\begin{itemize}
\item\textbf{Mean}\\ $$\mathbb{E}(y_t)=\mathbb{E}(\mu+\epsilon_t+\theta \epsilon_{t-1})=\mu+\mathbb{E}(\epsilon_{t})+\theta\mathbb{E}(\epsilon_{t-1})$$\\
...
\begin{itemize}
\item \textbf{Autocovariance}: the covariance between two values of the time series at different points in time. \\ 
\vspace{0.5cm}
...
```{r, echo=FALSE}
knitr::include_graphics("/Users/flaviapaoloni/Downloads/Intro-to-Time-Series/ma.pdf")
```


## Autoregressive process

Autoregressive (AR) process: the time series $y_t$ depends on its previous values and on an error term $\epsilon_t$.\medskip
...
\textcolor{blue}{\cmark} The stationarity condition holds \textbf{if} $|\phi|$ is lower than 1.

Let's consider an AR(1) process.  Its statistical properties are: 

\begin{itemize}\item \textbf{Mean}\\ $$\mathbb{E}(y_t)=\mathbb{E}(\mu+\phi y_{t-1}+\epsilon_t)=\mu+\phi\mathbb{E}(y_{t-1})$$\\
    If the stationarity condition holds, the mean of $y_t$ is the same for all $t$ $\implies$ $\mathbb{E}(y_t)=\mathbb{E}(y_{t-1})$. \\\medskip
...
\begin{itemize}\item \textbf{Variance}\\ $$\mathbb{V}ar(y_t)=\gamma(0)=\mathbb{V}ar(\mu+\phi y_{t-1}+\epsilon_t)$$ $$\mathbb{V}ar(y_t)=\mathbb{V}ar(\phi y_{t-1})+ \mathbb{V}ar(\epsilon_t)$$\\\medskip
    If the stationarity condition holds, the variance of $y_t$ is the same for all $t$ $\implies$ $\text{$\mathbb{V}$ar}(y_t)=\text{$\mathbb{V}$ar}(y_{t-1})$. \\\medskip
...
\begin{itemize}\item \textbf{Autocovariance}: the covariance between two values of the time series of order one is:\\ $$\text{$\mathbb{C}$ov}(y_t, y_{t-1})=\gamma(1)=\text{$\mathbb{C}$ov}(\mu+\phi y_{t-1}+\epsilon_t, y_{t-1})$$
...
```{r, echo=FALSE}
knitr::include_graphics("/Users/flaviapaoloni/Downloads/Intro-to-Time-Series/ar.pdf")
```
\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{figures/ar.pdf}
    \caption{\small The coefficient $\phi$ is set to 0.7}
    %\label{fig:my_label}
\end{figure}    
...
\textcolor{blue}{\ding{51}} The stationarity condition holds \textbf{if} $|\phi|$ is lower than 1.

Notice that ARMA(1,0) is an AR(1) and ARMA(0,1) is an MA(1).

\begin{itemize}
    \item \textbf{Mean} $$\mathbb{E}(y_t)= 0$$ If there is an intercept $\mu$ in ARMA(1,1), the mean is $$\mathbb{E}(y_t)= \frac{\mu}{1-\phi}$$
    \item \textbf{Variance}:$$\text{$\mathbb{V}$ar}(y_t)=\gamma(0)=\text{$\mathbb{V}$ar}(\phi y_{t-1}+\epsilon_t+\theta \epsilon_{t-1})$$
...
\begin{figure}
   ...
   \caption{\small The coefficient $\phi$ and $\theta$ are both set to 0.7}
    %\label{fig:my_label}
\end{figure}   


```{r, echo=FALSE}
knitr::include_graphics("/Users/flaviapaoloni/Downloads/Intro-to-Time-Series/arma.pdf")
```
...
\textcolor{red}{\ding{55}} The stationarity condition \textbf{doesn't} hold (see next slide).

Note that $\epsilon_t \; \forall t$ is a white noise process, $\epsilon_t \sim \mathcal{N}(0, \sigma^2)$.\medskip

\begin{itemize}
    \item \textbf{Mean} $$\mathbb{E}(y_t)=\mathbb{E}(\epsilon_1+\ldots+\epsilon_t)=0$$ 
    \item \textbf{Variance} $$\text{$\mathbb{V}$ar}(y_t)=\text{$\mathbb{V}$ar}(\epsilon_1)+\ldots+\text{$\mathbb{V}$ar}(\epsilon_t)=t\sigma^2$$
\end{itemize}
The variance increases with $t$ $\implies$ the variance depends on time $\implies$ non-stationary process.

\begin{itemize}
    \item \textbf{Covariance} $$\text{$\mathbb{C}$ov}(y_t, y_{t-h})= \text{$\mathbb{C}$ov}(y_{t-h}+\epsilon_{t-h+1}+\ldots+\epsilon_t, y_{t-h})$$
...
```{r, echo=FALSE}
knitr::include_graphics("/Users/flaviapaoloni/Downloads/Intro-to-Time-Series/rw.pdf")
```

\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{figures/rw.pdf}
   % \caption*{\small The coefficient $\phi$ is set to 0.7}
    %\label{fig:my_label}
\end{figure}   
...
\begin{align*}
    y_t&=\phi y_{t-1}+\epsilon_t
\end{align*}
\textbf{Focus}: the coefficient $\phi$. Is it equal to 1 (unit root) or smaller than 1?\medskip
...
\end{align*}
where $\hat{\phi}$ is the parameter $\phi$ of AR(1) estimated using OLS. \\\medskip
...
# Analysis of Economic Variables
   Exogenous shocks are unpredictable events affecting a system of variables. For example, a monetary policy shock affects a country's output and inflation.\medskip

   ARMA models are useful to analyse the response of economic variables to these shocks.\medskip
   
   \textcolor{blue}{\hyperlink{irfs}{Impulse Response Functions (IRFs)}} estimate the expected response of these variables, modelled with ARMA processes, to a shock.\medskip
...
\begin{figure}
    \centering
    \includegraphics[width=0.8\linewidth]{figures/irfshock.png}
    \caption{Impulse/shock: Fed Funds interest rate, Response: GDP}
...
\begin{align}
    ...
    \widetilde{y}_t&=y+\frac{\partial y_t}{\partial \epsilon_s}\cdot 1 \;\;\;\;\; \forall t\geq s\\\label{eq:partirf}
\end{align}

\@ref(eq:partirf).

\textbf{Focus}: the response of the time series $y$ to this exogenous shock. \\
\medskip
...
## Local Projections
The Local Projection (LP) methods are useful to compute IRFs addressing possible misspecification in the model and considering multiple variables. \medskip
...
Our goal is to estimate the effect of this increase on GDP in the next quarters/years.\\\medskip
At time $t+1$, estimate the regression in eq. \eqref{lp} with 1 lag using OLS:
\begin{align}
...
\textbf{Focus}: the estimated $\beta_s$. \\
$\beta_s$ represents the impact of the shock on the variable $y_t$ - so it is the Local Projection impulse response.\medskip

Estimate the equations for both time series and for each step ahead $s=1,2,3,\ldots$ using OLS. \medskip
...
\begin{equation}
    \beta_s=\mathbb{E}(y_{t+s} |x_t=1,y_t,x_t) - \mathbbE(y_{t+s} |x_t =0,y_t,x_t)
\end{equation}
\textbf{Note}: when estimating, include heteroskedasticity- and autocorrelation-consistent (HAC) standard errors. 
